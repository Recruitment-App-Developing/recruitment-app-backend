...
public class JobController {
    ...
    public ResponseEntity<MetaResponse<MetaResponseDTO, List<EmployerJobResponseDTO>>> getListJobByCompany(
            @ParameterObject MetaRequestDTO metaRequestDTO, @ParameterObject SearchJobByCompanyRequestDTO requestDTO) {
        Integer accountId = AuthUtil.getRequestedUser().getId();
        return ResponseEntity.status(HttpStatus.OK)
                .body(jobService.getListJobByCompany(requestDTO, accountId, metaRequestDTO));
    }
    ...
    public ResponseEntity<MetaResponse<MetaResponseDTO, List<RelatedJobResponseDTO>>> findListJobByCompany(
        ...
        return ResponseEntity.status(HttpStatus.OK)
                .body(jobService.findListJobByCompany(metaRequestDTO, companyId, jobName, address));
    }
    ...
    public ResponseEntity<Response<DetailJobPageResponseDTO>> getDetailJobPage(
    ...
    public ResponseEntity<Response<DetailJobResponseDTO>> updateJob(
            @PathVariable(name = "jobId") Integer jobId, @RequestBody UpdJobRequestDTO requestDTO) {
        return ResponseEntity.status(HttpStatus.OK)
                .body(Response.successfulResponse(
    ...
    public ResponseEntity<Response<List<JobAddressResponseDTO>>> getListJobAddressByJob(
        ...
        return ResponseEntity.status(HttpStatus.OK)
                .body(Response.successfulResponse(
                        ...
                        jobService.getListJobAddressByJob(
                                AuthUtil.getRequestedUser().getId(), jobId)));
    }

    @PostMapping(Endpoint.V1.Job.UPDATE_JOB_ADDRESS)
    public ResponseEntity<Response<List<JobAddressResponseDTO>>> updateJobAddress(
            @PathVariable(name = "jobId") Integer jobId, @RequestBody JobAddressRequestDTO requestDTO) {
        return ResponseEntity.status(HttpStatus.OK)
                .body(Response.successfulResponse(
                        "Update job address successfull",
                        jobService.updateJobAddress(
                                requestDTO, AuthUtil.getRequestedUser().getId(), jobId)));
    }
    ...
    public ResponseEntity<Response<List<JobAddressResponseDTO>>> deleteJobAddress(
        ...
        return ResponseEntity.status(HttpStatus.OK)
                .body(Response.successfulResponse(
                        ...
                        jobService.deleteJobAddressId(
                                AuthUtil.getRequestedUser().getId(), jobId, jobAddressId)));
    }
}
