...
public interface CandidateRepository extends JpaRepository<Candidate, Integer> {
    // CANDIDATE
    @Query(value = "SELECT c FROM Candidate c " + "JOIN FETCH c.avatar " + "JOIN FETCH c.address " + "WHERE c.id = :id AND c.deleted = false")
    Optional<Candidate> findNoDeletedAccountById(@Param(value = "id") Integer id);
