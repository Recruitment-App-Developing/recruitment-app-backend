...
public interface JobRepository extends JpaRepository<Job, Integer> {
    ...
    @Query(value = "SELECT j FROM Job j " +
            "JOIN JobAddress ja ON ja.job.id = j.id " +
    ...
    Page<Job> findListJobByCompany(Pageable pageable, @Param("companyId") Integer companyId,
                                   @Param("companyName") String companyName, @Param("address") String address);
//    @Query(value = "SELECT new com.ducthong.TopCV.repository.objects.StatisticGeneralJobByIndustryObject( i.id, i.name, COUNT(j.id)) " +
