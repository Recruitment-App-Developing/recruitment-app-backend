...
public class JwtTokenUtil {
    ...
    public String generateToken(Account account){
        Map<String, Object> map = Map.ofEntries(
                Map.entry("id", account.getId().toString())
        );
        ...
        return Jwts.builder()
                ...
                .setExpiration(new Date(
                        Instant.now().plus(
                                JWT_EXPIRE_SECONDS, ChronoUnit.SECONDS)
                                .toEpochMilli()))
    ...
    public String getAccountId(String bearerToken){
        return (String) this.extractAllClaims(bearerToken.substring(BEARER_TOKEN.length())).get("id");
    }
